...
export default function EventEditPage() {
    ...
    if (eventId) {
      fetch(`/api/events/${eventId}`)
        ...
        .then(data => {
          ...
          // 曜日・時間帯・期間・カレンダー状態もAPIレスポンスにあれば反映
          if (Array.isArray(data.selectedWeekdays)) setSelectedWeekdays(data.selectedWeekdays);
          if (data.bulkTimeSlot) setBulkTimeSlot(data.bulkTimeSlot);
          if (data.customStartTime) setCustomStartTime(data.customStartTime);
          if (data.customEndTime) setCustomEndTime(data.customEndTime);
          if (data.bulkStartDate) setBulkStartDate(data.bulkStartDate);
          if (data.bulkEndDate) setBulkEndDate(data.bulkEndDate);
          ...
          if (data.individualTimeType) setIndividualTimeType(data.individualTimeType);
          if (data.individualStartTime) setIndividualStartTime(data.individualStartTime);
          if (data.individualEndTime) setIndividualEndTime(data.individualEndTime);
          setLoading(false);
        })
        .catch(err => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // CandidateDate[]をstring[]に変換
    const formattedDates = candidateDates.map(cd => `${cd.date}T${cd.time}`);
    
    const res = await fetch(`/api/events/${eventId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ title, description, candidateDates: formattedDates }),
    });
    
    if (res.ok) {
      setSaveMessage("✅ イベントが更新されました！");
      setTimeout(() => {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ marginBottom: 24 }}>
          <input 
            ...
            required 
            style={{ width: "100%", marginBottom: 12, padding: 8, borderRadius: 6, border: "1px solid #a5d6a7" }} 
          />
          <textarea 
            placeholder="詳細" 
            value={description} 
            onChange={e => setDescription(e.target.value)} 
            style={{ width: "100%", marginBottom: 12, padding: 8, borderRadius: 6, border: "1px solid #a5d6a7", minHeight: 80 }} 
          />
        </div>
        ...
        <div style={{ marginBottom: 32, padding: 16, border: "2px solid #a5d6a7", borderRadius: 8 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
            <div>
              ...
              <input 
                ...
                style={{ width: "100%", padding: 4, marginBottom: 8, borderRadius: 4, border: "1px solid #a5d6a7" }}
              />
              <input 
                type="date" 
                value={bulkEndDate} 
                onChange={e => setBulkEndDate(e.target.value)}
                style={{ width: "100%", padding: 4, borderRadius: 4, border: "1px solid #a5d6a7" }}
              />
            </div>
          ...
          <button 
            type="button" 
            onClick={generateBulkDates}
            disabled={selectedWeekdays.length === 0}
            style={{ 
              padding: "8px 16px", 
              background: selectedWeekdays.length > 0 ? "#81c784" : "#ccc", 
              color: "#fff", 
              border: "none", 
              borderRadius: 4,
              cursor: selectedWeekdays.length > 0 ? "pointer" : "not-allowed"
            }}
          >
            候補日を生成
          </button>
        ...
        <div style={{ marginBottom: 32, padding: 16, border: "2px solid #a5d6a7", borderRadius: 8 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <button type="button" onClick={() => changeMonth(-1)} style={{ padding: "4px 8px", border: "1px solid #a5d6a7", borderRadius: 4, background: "#fff" }}>
              ←
            </button>
          ...
          {selectedDate && (
            <div style={{ padding: 16, background: "#e8f5e8", borderRadius: 6 }}>
              <div style={{ marginBottom: 12 }}>
                <strong>選択した日付: {formatDate(selectedDate)}</strong>
              </div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button type="button" onClick={addIndividualDate} style={{ padding: "8px 16px", background: "#81c784", color: "#fff", border: "none", borderRadius: 4 }}>
                  候補日に追加
                </button>
                
                {/* プリセットボタン */}
                <button type="button" onClick={() => addTimePreset('morning')} style={{ padding: "6px 12px", background: "#e8f5e8", border: "1px solid #a5d6a7", borderRadius: 4, cursor: "pointer" }}>
                  午前で追加
                </button>
                <button type="button" onClick={() => addTimePreset('afternoon')} style={{ padding: "6px 12px", background: "#e8f5e8", border: "1px solid #a5d6a7", borderRadius: 4, cursor: "pointer" }}>
                  午後で追加
                </button>
                <button type="button" onClick={() => addTimePreset('allday')} style={{ padding: "6px 12px", background: "#e8f5e8", border: "1px solid #a5d6a7", borderRadius: 4, cursor: "pointer" }}>
                  終日で追加
                </button>
              </div>
            </div>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          {candidateDates.length === 0 ? (
            <div style={{ padding: 16, textAlign: "center", color: "#666", background: "#f9f9f9", borderRadius: 6 }}>
              曜日時間帯一括指定またはカレンダー個別指定で候補日を追加してください
            </div>
          ) : (
            <div style={{ border: "1px solid #a5d6a7", borderRadius: 6 }}>
              {sortedCandidates.map((candidate, index) => (
                <div key={index} style={{ display: "flex", alignItems: "center", padding: 12, borderBottom: index < sortedCandidates.length - 1 ? "1px solid #e0e0e0" : "none" }}>
                  ...
                  <button
                    type="button"
                    onClick={() => removeCandidate(candidateDates.indexOf(candidate))}
        ...
        <button 
          ...
          style={{ 
            width: "100%", 
            background: candidateDates.length > 0 ? "#81c784" : "#ccc", 
            color: "#fff", 
            border: "none", 
            borderRadius: 6, 
            padding: 12, 
            fontWeight: "bold", 
            fontSize: 16,
            cursor: candidateDates.length > 0 ? "pointer" : "not-allowed"
          }}
        >
          イベントを更新 ({candidateDates.length}件の候補日)
        </button>
      </form>
      
      {saveMessage && (
        <div style={{ marginTop: 16, padding: 16, background: "#e8f5e8", borderRadius: 6, textAlign: "center" }}>
